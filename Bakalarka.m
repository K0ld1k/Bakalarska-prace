...
Ms1 = [400 410 420 430 450 460;   %x
        20  60  40 -20  40   0;   %y
        10   4   2   5   5   3];  %radius

Ms = [200 200 200 300 300 400;   %x
      -70  0  70 -30  30   0;   %y
       20  20  20  20  20  20];  %radius
target = [600,0];
...
figure
simulation_plot(x,y,neighbors,target,Ms,agents)
axis([-200 700 -200 200])
...
for iteration = 1:length(t)
    ...
    if(iteration > 1)
       velocity_it(iteration,:) = ((nodes_vel1(:,1).^2)+(nodes_vel1(:,2).^2));
    ...
    for i = 1:1:agents
        ...
        for k = 2:1:neigh_len2
            ...
            sigma1 = sigma(z);                
            grad_alpha(i,:) = grad_alpha(i,:) + (Phi_alpha(sigma1,r,d) * nij_val(n2,:)); 
        end
    end
    ...
    con_mat_a = cons_alpha_mat(x,y,neighbors,r,agents);
    for i = 1:1:agents
    ...
    for k = 1:1:length(q_ik)
        ...
        for j = 1:1:agents
            ...
            sig1 = sigma(z);
            grad_beta1(j,:) = Phi_alpha(sig1,r_b,d_b) * nik_val(j,:);
        end
    ...
    con_mat_b = cons_beta_mat(x, y, q_ik, d_b, agents);
    ...
    if(xy_center(1) > target(1)-0.1 && xy_center(1) < target(1)+0.1 && xy_center(2) > target(2)-0.1 && xy_center(2) < target(2)+0.1 && rank(adj_mat) == agents)
        sim_end = iteration;
        break
    end
...
%% Graf trajektorie
figure
for i = 1:1:length(t(1,1:sim_end))
    if(i == length(t(1,1:sim_end)))
        plot(x_iter(:,i),y_iter(:,i),'bluediamond');
    else
...
figure
hold on
plot(t(1,1:sim_end),ua_iter(1,1:sim_end))
plot(t(1,1:sim_end),ua_iter(2,1:sim_end))

plot(t(1,1:sim_end),ub_iter(1,1:sim_end))
plot(t(1,1:sim_end),ub_iter(2,1:sim_end))

plot(t(1,1:sim_end),uc_iter(1,1:sim_end))
plot(t(1,1:sim_end),uc_iter(2,1:sim_end))
title("Řízení agentů")
xlabel("Iterace")
ylabel("Vstup řízení")
...
for i = 1:length(values)
    ph_values(i) = ph(values(i),0.2); 
end
plot(values,ph_values)
hold on
for i = 1:length(values)
    ph_values(i) = ph(values(i),1); 
end
plot(values,ph_values)
grid on
box on
...
xlabel("z")
ylabel("\rho_h(z)")
legend("Nárazová funkce (h=0.2)","Nárazová funkce (h=1)")
...
for i = 1:length(values1)
    phi_alpha_values(i) = Phi_alpha(values1(i),1.2*10,10,0.2); 
end
plot(values1,phi_alpha_values)
hold on
for i = 1:length(values1)
    phi_alpha_values(i) = Phi_alpha(values1(i),1.2*10,10,0.5); 
end
plot(values1,phi_alpha_values)
grid on
box on
...
xlabel("z")
ylabel("\phi_\alpha(z)")
legend("Akční funkce (h=0.2)","Akční funkce (h=0.5)")
%%
figure
